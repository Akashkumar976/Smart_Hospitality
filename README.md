<img width="809" height="648" alt="Screenshot 2025-12-07 141749" src="https://github.com/user-attachments/assets/a321a60d-26b2-4cf6-8497-83bb3f8d7758" /><img width="1883" height="815" alt="Screenshot 2025-12-07 141734" src="https://github.com/user-attachments/assets/32e6f15f-b499-4f14-b5f9-a5a7881ec154" /><img width="1289" height="635" alt="Screenshot 2025-12-07 141710" src="https://github.com/user-attachments/assets/12b45410-e55b-42de-8ecd-34a321610d15" /><img width="1290" height="667" alt="Screenshot 2025-12-07 141654" src="https://github.com/user-attachments/assets/bd4f22ac-f8ce-4816-af84-c36b5ed588d0" /># 🏥 Smart Hospitality – Complete Hospital Management System

Smart Hospitality is a secure and modern **Hospital Management System (HMS)** designed to automate and streamline hospital operations.  
It provides role-based access, real-time bed availability, automated billing, appointment scheduling, and secure staff management.

🚀 **Live Demo:**  
👉 https://smart-hospitality-4.onrender.com/
**My portfolio**
https://akashsingh.onrender.com/

> ⚠ Free tier deployment (Render): Server sleeps after 15 minutes of inactivity and may take **~2 minutes** to wake up.

---

## 👤 Demo Login Credentials

| Role | Staff ID | Password |
|------|----------|-----------|
| Staff User | **STF027** | **12345** |

---

# 🌟 Features

### 🧑‍🤝‍🧑 Patient Management
- Add new patients with auto-generated **unique Patient ID**
- Update patient details
- Track patient statuses: **NEW → ADMITTED → DISCHARGED**
- Smooth search and filtering

### 🛏 Bed Management
- Smart **real-time bed allocation**
- Prevents double-allocation using safety checks
- Bed categories (ICU, General, Private, Emergency)
- Automatic bed release on discharge

### 📅 Appointment Scheduling
- Doctor-wise appointment slots
- Status tracking for appointments
- Simple and clean UI for staff and doctors

### 💰 Billing System
- Auto-generated billing on discharge
- Includes bed charges, doctor fees, and medicine costs
- Printable bills

### 🔐 Secure Staff Login
- Staff login using:
  - **Staff ID**
  - **Password**
- Fully session-based security with restricted access

### 🧑‍💼 Admin Dashboard
- Only **admin** can add staff
- Mandatory **Aadhaar verification** during staff creation
- Manage doctors, staff, roles, and permissions

### 📱 Fully Responsive UI
- Designed for mobile devices
- Smooth, fast, and user-friendly interface

---

# 🧰 Tech Stack

| Layer | Technology |
|-------|------------|
| Backend | **Spring Boot 3** |
| Frontend | HTML, CSS, JavaScript |
| Deployment | **Docker**, Render |
| Database | MySQL |
| Security | Spring Security, Session Auth |

---

# 📸 Screenshots 

<img width="1237" height="886" alt="Screenshot 2025-12-07 141641" src="https://github.com/user-attachments/assets/0c41ff69-16c9-401e-a6af-60df8e677b8e" />
<img width="1823" height="863" alt="Screenshot 2025-12-07 141624" src="https://github.com/user-attachments/assets/eadc66af-79d1-4d2c-9cda-ebc78443a427" />
<img width="771" height="639" alt="Screenshot 2025-12-07 141606" src="https://github.com/user-attachments/assets/44faf2b5-70c5-4d94-8830-ba4b669fc417" />
<img width="1848" height="888" alt="Screenshot 2025-12-07 141556" src="https://github.com/user-attachments/assets/076d7fc2-1c63-4c9e-9905-0fb628f79e82" />
<img width="1673" height="902" alt="Screenshot 2025-12-07 141520" src="https://github.com/user-attachments/assets/eb9da7b0-2ef2-44e6-b352-232298158993" />
<img width="1875" height="887" alt="Screenshot 2025-12-07 141454" src="https://github.com/user-attachments/assets/bcaf2c7c-fcf7-4a95-9a8c-7aad46b9e825" />
<img width="1907" height="892" alt="Screenshot 2025-12-07 141436" src="https://github.com/user-attachments/assets/434cc4b8-0a1f-48a1-9563-c3cecf823406" />
<img width="1831" height="901" alt="Screenshot 2025-12-07 141418" src="https://github.com/user-attachments/assets/05328da5-82bd-4338-90ec-00a6dc326075" />
<img width="979" height="618" alt="Screenshot 2025-12-07 141806 - Copy" src="https://github.com/user-attachments/assets/4040e8d4-ce59-41a7-b578-1b780b3f883e" />

